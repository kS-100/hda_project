...
def create_model_folder(model_id):
    model_dir = os.path.join(BASE_MODEL_PATH, model_id)
...
def evaluate_model(model, x_train, y_train, x_test, y_test, model_name, features_to_show=20,
                   show_scores=True, show_plots=True):
    # create folder for the model data.
...
def persist_model(model, model_dir, model_name='model.joblib'):
    create_model_folder(model_dir)
    # serialize the model
    dump(model, get_model_path(model_dir, model_name))
